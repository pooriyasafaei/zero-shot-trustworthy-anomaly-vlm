...
def pvalue_uncertainty(p: np.ndarray, mode: str = "symmetric", delta: float = 0.05) -> np.ndarray:
    ...
    not an ambiguous one. The rejection boundary sits at ``p = delta``. We keep
    the specified forms as the default for comparability with the proposal, but
    ``boundary`` is the variant whose maximum coincides with where errors
    actually live, and the two are compared head-to-head in the bake-off.
    """
...
class MondrianConformal:
    ...
    def transform(self, test: pd.DataFrame, score_col: str = "anomaly_score",
                  group_col: str = "category", uncertainty_mode: str = "symmetric") -> pd.DataFrame:
